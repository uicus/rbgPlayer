...
node_address tree::reparent_along_move(const reasoner::move& m){
    ...
    state_tracker tracker(cache, nodes_register, root_state);
    mitigate_pointers_invalidation_during_expansion();
    const auto move_index = nodes_register[root_index].get_node_index_by_move(m, tracker); // TODO: not single index but vector of them
    const auto& new_root = nodes_register[root_index].get_node_by_address({move_index}, tracker); // TODO: same
    root_state = tracker.get_state();
...
game_status_indication tree::get_status(uint own_index){
    state_tracker tracker(cache, nodes_register, root_state);
    if(nodes_register[root_index].is_terminal(tracker)) // TODO: do it in some more crude way
        return end_game;
